...
\begin{itemize}
	...
	\item \textbf{Bradycardia Arrhythmia:} A condition one has a Resting Heart Rate (RHR) that is abnormally slow. The question of “how slow is abnormally slow?” may differ	from individual to individual because the heart’s RHR tends to decrease with age and 7 among individuals with higher fitness levels. For most adults, a slow RHR is less than 50 or 60 Beats per Minute (BPM). Symptoms of bradycardia may include fatigue, dizziness,	fainting, heart palpitations and breathing problems.
...
A pacemaker system is typically composed of two main sub-systems: the pulse generator and the leads. The pulse generator, also known as the "can", contains the battery of the pacemaker and the circuits that deliver the pacing stimuli. It provides protection for the internal circuits of the pacemaker and the patients heart from each other. There is a connector from the pulse generator to the leads sub-system. The leads are insulated wires that carry electrical stimuli from the pulse generator to the heart. They are the first component of the pacemaker that is implanted during surgery, usually through the subclavian vein. The electrodes on the end of the lead are they attached to the heart.
...
The full requirement set by Boston Scientific for a pacemaker~\cite{scientific2007pacemaker} is available in the Appendix~\ref{apdx:Req}. Though not explicitly stated as domain analysis, there is some knowledge capture that occurs in chapters 1 and 2 of the pacemaker requirements all captured in natural language. In fact, all the requirements specified by Boston Scientific are captured in natural language. These two chapters introduce the scope of the requirements, identify some of the stakeholders, and define the system and sub-systems. We found it sufficient for the engineers to have an idea of why the system is needed, who they are building it for, and where its intended usage environment is. This comes from anecdotal accounts from engineers in industry. They want to be able to justify their time and effort into building a product that has a purpose, even if they do not have all the details on the finish product. We now attempt to improve this domain analysis using the goal diagram and \gls{UCD} approaches outlined in our proposed methodology.
...
\begin{itemize}
	...
	\begin{itemize}
		\item A follow-up question for was what goals the identified stakeholder would have?
	\end{itemize}
...
\begin{enumerate}
	...
	\item \textbf{Explant:} This stage is where a pacemaker is removed from the patient. A cost-benefit analysis is performed with the patient to assess if removal of the pacemaker is more beneficial instead of leaving it in. The pacemaker may also be removed after patients death.

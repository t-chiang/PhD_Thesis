...
Throughout this thesis we have used the automotive domain as the basis for illustrative examples and to motivate the design of \tool\ and its features. This is because within the automotive domain we can find two of the main issues we are addressing with this methodology; how to handle variants within a product family and maintain traceability throughout the product family. For automotive manufacturers there are variants in the customer facing product, evident in the different vehicles that are offered, the different model years between vehicles, and the various trims that are available within a single vehicle model. There are also variants that can exist internal to the manufacturer as they go through development and decompose the vehicle into its components: engine, transmission, cooling, heating variants that are used across platforms for example. Managing these product families, across multiple levels of abstraction necessitates product family traceability in order to keep track of requirements constraints that lead to the various design and implementation decisions. 

For the evaluation portion of this thesis, we look towards the medical domain and medical device development. There is significant overlap overlap between medical device development and automotive manufacturing as they both represent safety critical systems and have to follow various standards and regulations across geographic regions to sell their products to customers. The medical device we will be using for this evaluation is a pacemaker. From the perspective of safety criticality, medical devices such as a pacemaker tend to have a greater impact on individual health and safety which is often reflected in medical regulations. Regional legislation may affect product development and the products themselves may have variants based on the capabilities of target stakeholders, in this case individual patients. As this research has been carried out in Canada, we reference medical device guidelines and classifications based on Health Canada regulations~\cite{CanadaGuidanceDocument, VerifyDevices} as we are from a Canadian institution. According to these guidelines, an implantable rate responsive pacemaker is classified as a class 4 medical device. This is the most critical classification a medical device can receive as it is considered a closed loop system used to control the treatment of a patients condition. Thus, a pacemaker has high safety-critical implications. This justifies high levels of scrutiny for the requirements and potential variations as there must be a great deal of confidence in the product lifeycle before being implanted in a patient.

This evaluation will use an existing requirement document from Boston Scientific~\ref{apdx:Req} for pacemaker development. We will be assessing how this methodology and \tool\ can be used to improve an existing requirement set and create traceability between features and requirement. We will also potential integration points with development processes to evaluate the adaptability of the methodology and \tool\ to minimally disrupt existing documentation and development strategies. 
...
The first step in this thesis is examining how the domain analysis is conducted. This part of the evaluation examines how we can enhance existing knowledge capture approaches by introducing domain analysis techniques introduced in our methodology. Using the proposed processes, along with \tool, we will be assessing the ability to find gaps in the original pacemaker requirement document, potential integration points for existing development process, and general enhancements for engineering practices.

We also want to identify what a pacemaker is. As we have previously identified according to Canadian guidelines~\cite{CanadaGuidanceDocument, VerifyDevices}, and based on the requirements in the Boston Scientific requirements document~\ref{apdx:Req}, we have an implantable rate responsive pacemaker. The purpose of this pacemaker is to treat various forms of cardiac arrhythmia, atypical beatings of a heart. There are three types of arrhythmia:
\begin{itemize}
	\item \textbf{Arrhythmia:} A condition wherein the heart beat irregularly or abnormally
	\item \textbf{Bradycardia Arrhythmia:} A condition one can have is a Resting Heart Rate (RHR) that is abnormally slow. The question of “how slow is abnormally slow?” may differ	from individual to individual because the heart’s RHR tends to decrease with age and among individuals with higher fitness levels. For most adults, a slow RHR is less than 50 or 60 Beats per Minute (BPM). Symptoms of bradycardia may include fatigue, dizziness,	fainting, heart palpitations and breathing problems.
	\item \textbf{Tachycardia Arrhythmia:} A condition wherein one has a RHR that is abnormally fast. For most adults, a fast RHR is above 100 BPM.
\end{itemize}

A pacemaker system is typically composed of two main sub-systems: the pulse generator and the leads. The pulse generator, also known as the ``PG", contains the battery of the pacemaker and the circuits that deliver the pacing stimuli. It provides protection for the internal circuits of the pacemaker and the patients heart from each other. There is a connector from the pulse generator to the leads sub-system. The leads are insulated wires that carry electrical stimuli from the pulse generator to the heart. They are the first component of the pacemaker that is implanted during surgery, usually through the subclavian vein. The electrodes on the end of the lead are they attached to the heart.

Finally, as this is an implantable closed loop system, there is a Device Controller-Monitor system that is paired with the pacemaker. This device is used by the physicians, nurses, and technicians to interrogate and alter the behaviour of the pacemaker device according to patient needs. It provides a means of remotely transmitting programming data to the pacemaker while implanted and receiving transmitted telemetry data from the pacemaker.

A more in-depth description of what a pacemaker is can be found in the appendix~\ref{apdx:What_Is_A_Pacemaker}. The document goes into greater detail of the Cardiac Conductive System and how it interacts with the pacemaker.

%\gls{MDE} concepts to support later stages of development, including traceability and iteration.

\subsection{Goal Diagrams}

The full requirement set by Boston Scientific for a pacemaker~\cite{scientific2007pacemaker} is available in the Appendix~\ref{apdx:Req}. Though not explicitly stated as domain analysis, there is some knowledge capture that occurs in chapters 1 and 2 of the pacemaker requirements all captured in natural language. In fact, all the requirements specified by Boston Scientific are captured in natural language. These two chapters introduce the scope of the requirements, identify some of the stakeholders, and define the system and sub-systems. We found it sufficient for the engineers to have an idea of why the system is needed, who they are building it for, and where its intended usage environment is. This comes from anecdotal accounts from engineers in industry. They want to be able to justify their time and effort in building a product that has a purpose, even if they do not have all the details of the finished product. We now attempt to improve this domain analysis using the goal diagram and \gls{UCD} approaches outlined in our proposed methodology.
...
\begin{enumerate}
	\item \textbf{Pre-Implant:} This stage is where the patient is assessed if a pacemaker is required for treatment. This stage includes risk/benefit discussion, patient risk tolerance, and informed patient consent. The patient has right to refuse care and may deny getting a pacemaker altogether. It may also be determined that though the patient may need a pacemaker, it would not be to their benefit to have it implanted.
	\item \textbf{Implant:} This stage is the surgery. The patient undergoes surgery to implant the pacemaker.
	\item \textbf{Follow-up:} This stage occurs directly after the pacemaker is implanted. The patient is assessed for surgery safety (correct implantation of the pacemaker, no infection, no other surgical complications etc). This is the follow-up assessment to make sure that the surgery was a success and there are no unforeseen complications after the surgery is complete.
	...
	\item \textbf{Explant:} This stage is where a pacemaker is removed from the patient. A cost-benefit analysis is performed with the patient to assess if removal of the pacemaker is more beneficial instead of leaving it in. Also, the pacemaker has to be removed after a patient's death.
	\item \textbf{Disposal:} This stage is when a pacemaker has reached end of life and is safely disposed.
\end{enumerate}

These definitions are helpful to engineers to get a better understanding of the complete product lifecycle after development. It helps to inform some of the goals that the various stakeholders may have, providing some insight into why the requirements are specified as such. Further, these extra definitions also helped us to identify more stakholders of the system beyond the original specification. Some new stakeholders were also identified during the interview with a doctor that were not captured in the original pacemaker requirement document. The original list of stakeholder is the following:
...
After following through on the domain analysis, we produce an extended list of stakeholders:
...
The patient family was identified as another stakeholder during our interview with the doctor. They also share some of the goals that were identified for both the patient and doctor stakeholders. One of the goals for doctors is to help the patient family navigate the health care system. Identifying this goal reinforced the idea that the patients family are also stakeholders of the pacemaker system as they are likely to be involved with the patient quality of life. This ties into another goal we identified for the patient which is to regain or maintain a desired quality of life. The goals of the doctor and the patient stakeholders can be found in figures~\ref{fig:doctor_goals} and~\ref{fig:patient_goals} respectively.
...
We also identify what the goals are for the nurse and technician stakeholders. This helps to clarify what roles they would play in the context of the pacemaker lifecycle. During the ambulatory stage, the patient will be spending a lot of time with the nurse to monitor health and functionality of the pacemaker. The technician may be called if there is a bug detected for troubleshooting, and in extreme cases, disposal of the pacemaker. Disposal of a pacemaker requires that the pacemaker be returned to the manufacturer due to the potential of batteries exploding if improperly handled. We also add what the goals of the hospital could be, though the hospital is mostly where the resources are sourced. There can be more goals for the hospital in general, but for this context we limited the goals to the scope of a patient with bradycardia and the pacemaker lifecycle.
...
Thus far, the process of modelling our goals has produced two more stakeholders that were not identified in the original requirement document. We have provided insight into what some of the goals are for each of the stakeholders, along with supporting beliefs, tasks, and resources. We can claim that we have reduced some uncertainty around what goals the stakeholders would have in regards to the pacemaker product lifecycle. We also claim that we have provided some extra context around the stages of the pacemaker lifecycle that can help inform or support decisions for the requirements and subsequent development. 

\subsection{Use Case Diagrams}

The next step is to identify the use cases for each of these stakeholders, and compare them with the identified uses of the original requirement document. There is a variety of tasks (from the goal diagrams) that we have identified for the stakeholders that break down loosely into two categories; tasks that require the pacemaker and tasks that are human supplemented. According to the methodology, and because our primary objective is to develop the product, we want to identify the use cases that the stakeholders will need the pacemaker system for. These use cases should also be traceable to one or more goals for the stakeholders. This is critical to begin end-to-end traceability for the entire product lifecycle development. For our most immediate task, the traceability helps to justify the identified use cases based on stakeholder goals.

\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{pacemaker_device.png}
	\caption{Image of a pacemaker pulse generator on the left. Diagram of a pacemaker device with leads. Extracted from the supplement document `What Is A Pacemaker' found in \ref{apdx:What_Is_A_Pacemaker}.}
	\label{fig:pacemaker_device}
\end{figure}

In Chapter 2: System Definition for the pacemaker requirements document it defines the various components that compose the pacemaker system; the pacemaker device (also known as the pulse generator or PG), the Device Controller-Monitor and associated software, and the device leads that are implanted in the patient. Those three components become the boundaries of our UCDs. This is an easy integration point between out methodology and the existing documentation. We can save some re-work by reusing the system and sub-system decompositions from the original document. The document does not explicitly state what the features or use cases are for each of the devices, but there are overview sections that we can use as a starting point for determining what the use cases are.

The PG device overview provides the following information~\cite{scientific2007pacemaker} summarized from the PACEMAKER document found in~\ref{apdx:Req}:
\begin{itemize}
...
It is more challenging to have confidence in the UCD of the DCM system due to the combined nature of the hardware and software sub-systems specified in the requirements. They are coupled in section 2.2.2 of the PACEMAKER document which makes identifying what is a software use case and what is a hardware use case difficult as they are currently written. More information may come to light during the refinement however at this point in development it is difficult to gain more insight into how the sub-systems could be decoupled.
...
Based on the UCD for the \pgd, every use case is necessary for the system, and we identified the reuse of single and dual pacing between rate-adaptive modes. As a result, there is no potential for variability in the final product since all the features are mandatory to complete the system. This provides a starting point for developing the product. We do not take this model as complete as we expect to learn more about the system through the requirement refinement process. As we refine the model this present opportunities for iteration between the phases of this approach. As more features are identified, there can be benefits in iterating between the feature model and \gls{UCD} to explore how they relate to the actors and to other features in the system.

%The refinement process will allow us to iterate and increment on this model.

\subsection{Composition}

Composition is a key component of the modelling strategies for this methodology. It is through composition that we create feature-requirement encapsulation; every requirement is owned by a certain feature of the product. This prevents floating requirements that cannot be traced to a specific feature. It also implicitly forces each feature to be well-defined, at least based on the quality of the requirements that the feature owns. The composition relationship between features and requirements in this methodology is how we get our feature-requirement traceability as almost as a by-product of good engineering practice.

Due to the current definition of the metamodel for \tool, feature composition is slightly different. Since we state that all requirements are scoped by their owned features, we do not support cross cutting requirements. When a feature composition is required to define a product based on the feature model, it becomes more of a checklist rather than more traditional composition weaving. This is by design as we want each feature to be self contained with respect to their requirements. This does not exclude dependencies between features however. The validation of feature dependencies can still happen during the composition process if properly implemented. At this time, feature composition is not an automated process in \tool. It requires manual intervention by the user to select what features from the product family to add to a product variant they define. Despite being a manual process, the specification of the tool helps to ensure that when a feature is added to a product, all the owned requirements need to be added as well to get a valid result thanks to the natural semantics of composition in SysML~\cite{sysml2019omg}.

\section{Requirement Engineering and Refinement}

With a product family created from the UCD, we can start to explore the use cases and refining them towards more proper requirements. We do this by taking the use cases and refining them as a user story. A user story outlines the steps that a user (a stakeholder) would need to take when using a product (a use case) to achieve a goal or task as outlined from the goal diagrams. We use the following structure when refining a use case to a user story:
...
The steps outlined in the success scenario are the functional requirements for the use case. They outline what needs to happen for the actor to accomplish whatever task is outlined. The outcomes in the secondary scenarios provide alternative results of actors actions. These alternative outcomes can be safety hazards, security hazards, system malfunctions, or user errors. It is important for this step to make sure `what' the stakeholder has to do to achieve their goal is captured, not `how'.

The secondary scenarios outline the justifications for mitigating requirements in order to improve the chance that the actor will arrive to the success postcondition, in this case that the \pgd\ is correctly programmed and pacing the patients heart correctly and safely. In the context of our methodology, mitigating requirements are most commonly categorized into safety, constraint, or qualitative requirements. At this time we have not ruled out that a mitigating requirement may lead to a functional requirement, however based on our experiences we support the convention that they are a better fit for the non-functional requirement types. The secondary scenarios come together to answer the question of `how' the stakeholder will accomplish their objective, typically by creating requirements that reduce the chances of a stakeholder reaching a secondary scenario.

\section{Tool Supported Development}

Having completed many steps in this methodology, we have developed tool support for the following steps of the methodology. The tool support for \tool\ was developed using Sirius~\cite{viyovic2014sirius} based on the metamodel specification from~\ref{fig:metamodel}. We can see how the tool supports our objectives of traceability through our feature-requirement encapsulation and the implementation of the various relationships between model elements from the specification.

With these requirements identified, we can begin modelling them in the requirement canvas. In \tool, the requirement canvas is embedded in the features of the product family. This is the feature-requirement encapsulation that we outlined in the methodology. In Figure~\ref{fig:req_canvas_creation_UI}, we show how a user can create an embedded requirement canvas based on our tool implementation.
...
Another part of the requirement canvas in our tool implementation is the ability to assign test cases and reviewers to specified requirements. At this point in development, the purpose of those model elements is primarily bookkeeping. A further benefit of using the Gherkin syntax is the many existing libraries that support it testing. At this time, we do not natively support testing in the tool. We do however, want to be able to keep track of which requirements have been satisfied and which ones have not. Additionally, we also want to keep track of which ones have been reviewed and approved. As part of the requirement engineering process it is important to keep track of which requirements are ready to development, and which ones are still undergoing refinement. We show, based on our tool implementation, how this can appear for the pacemaker in Figure~\ref{fig:pacing_reqs_tests_reviews_export}. Naturally, once these model elements are added and connected to requirements, they are automatically updated in the traceability matrix as well.
...
At this stage we have created a functional behavioural specification for the ``Provide programmable pacing'' use case beyond what is in the original specification. We have justified our requirements using the use cases from the \gls{UCD} which are in turn supported by our goal diagrams. We have also identified qualitative and safety requirements that were not identified in the original specification. We have demonstrated flexibility in how we can specify these requirements using the Gherkin syntax. This has shown the benefit of a semi-formal approach as we can get an order to our functional requirements if desired to help with project planning and scoping. This also shows potential integration points with existing \gls{FDD} development methods. The natural language also makes the specification process relatively easy to learn and approach while allowing meaningful specifications to help development. Finally, we have shown that all of these efforts automatically create a traceability matrix for us with no extra steps or inputs by us during this process execution.

\section{Iterative and Incremental Change}

For the purpose of this methodology, iterative changes are thought of as cycles between development phases (features and requirements, requirements and design, design and implementation). The idea for this definition is that there are lessons learned when working through a subsequent development phase that can inform and create cause for repeating a development task from a previous phase. In the case of this methodology, if there is a need or gap that is identified during the requirement phase, it may create a cause for repeating one of the processes in feature modelling phase. A need or gap between the requirement modelling and feature modelling phase could be that there are requirements that do not properly fit into any features. It may require a review to ensure that the identified features are complete and correct. 

Incremental changes are thought of as changes within a single development phase. We used this definition for the sake of limiting the scope of what an incremental change entails. This was to help our development efforts for tool implementation, and simplify requirements around incremental changes compared to iterative changes. Currently, our tool implementation of \tool\ supports the maintenance of traceability matrices through incremental changes as we have defined them. As engineers work through the various steps within a development phase, they will be constantly adding, modifying, or removing artifacts until they reach a satisfactory result. We consider all of these incremental changes until they get enough completed to move onto the next phase of development.

\begin{sidewaysfigure}
	\centering
	\includegraphics[width=\textwidth]{pacemaker_traceability_1.png}
	\caption{Feature-requirement traceability during the middle of development for a pacemaker first half.}
	\label{fig:pacemaker_traceabilit1}
\end{sidewaysfigure}

\begin{sidewaysfigure}
	\centering
	\includegraphics[width=\textwidth]{pacemaker_traceability_2.png}
	\caption{Feature-requirement traceability during the middle of development for a pacemaker second half.}
	\label{fig:pacemaker_traceability_2}
\end{sidewaysfigure}

There are three traceability matrices that are supported in our implementation for \tool; project traceability, feature-feature traceability, and feature-requirement traceability. Project traceability and feature-requirement traceability are defined to focus on traceability of the requirement entities. Project traceability is the traceability matrix for the entire project. Feature-requirement traceability is a traceability matrix that is scoped to a single feature. The Feature-feature traceability matrix is represents the feature model that is defined within the project.

\begin{sidewaysfigure}
	\centering
	\includegraphics[width=\textwidth]{pacemaker_feature_traceability.png}
	\caption{Feature-feature traceability during the middle of development for a pacemaker.}
	\label{fig:pacemaker_feature_traceability}
\end{sidewaysfigure}

These matrices are a second syntactic representation of the modelling environments present within the tool implementation. Since they are semantically equivalent, any change to the requirements in the requirement canvas or features in the feature modelling environment are automatically reflected in their respective traceability matrix. The tabular environments, though somewhat limited, are also capable of defining new requirements. The limitations are that test cases and reviewers cannot be created in the tabular environment. This is by design as it forces users to manually create those traces explicit in the modelling environment and reflect on what test cases are being created and who is approving of the requirements.

We currently have limited support for iterative changes according to our definition. While changes that are made to an existing model will naturally be automatically reflected in the traceability matrices, the impact of the change cannot currently be assessed. Based on the metamodel for \tool, however, the mechanisms to support change impact analysis are all present. This is due to the nature of traceability that is inherently built into the model and the way we require explicit connections between elements in order for a model to be what is consider syntactically valid by our current conventions. A more in-depth change impact analysis that performs both syntactic and semantic analysis would be desirable to inspire more confidence in the results. 

\tool\ is also currently only scoped to feature and requirement modelling. It does not currently support \gls{UCD}s or goal diagrams within itself. As a result, we do not automatically generate and maintain traceability to those phases of development. While this does provide a challenge for our goal of end-to-end traceability, we wanted to focus on the feature-requirement traceability and generate more confidence in that portion due to the novelty of the feature-requirement encapsulation before adding in the goal to use case to feature to requirement traceability as there is less novelty in that specification. There also was not another obvious connection point between goals, use cases, and features the same way that we found we could use features to encapsulate requirements. This lack of completion also hampers our efforts to support iterative changes between the various steps of the methodology and phases of development. This also represent a limitation of the methodology. Because we do not connect goal diagrams and use case diagrams with our other modelling specifications, we lose another benefit of this methodology; traceability as a by product of good engineering practice. This is explored in more depth in the future work chapter as we want to explore how to effectively connect these four modelling strategies into a single specification. By having all the modelling environments connected within \tool, we will have more confidence in being able to trace iterative changes and change impact more comprehensively through the various stages of development. 

Overall, \tool\ shows a lot of promise for creating supplementing gaps that can exist in current requirement engineering practices. The methodology shows good support for traceability throughout the various stages of execution. Each step naturally flows into the next, and every subsequent step can be directly supported and traced to preceding work. Currently, there is a gap that exists between the goal diagram, the \gls{UCD} and the feature modelling and requirement modelling environment due to a limitation in scope for the initial specification. Despite this current limitation, we have shown the potential for traceability between the various modelling environments. The current challenge is finding the optimal mechanics for connecting the various modelling environments together.

Further, once all the modelling environments can be traced to each other mechanically, that will further the goal of a more comprehensive change impact analysis. It will also allow for end-to-end traceability to support argumentation and justification for safety and security of the product being specified helping with safety and security assurance. In this current iteration of \tool, we have shown how we can automate traceability between features and requirements. We have also justified our various classifications of requirements, in line with previous work, while further refining them to help with specific development goals for the product. We have shown how to flow from domain analysis through to requirement elicitation and how to justify each decision along the way. 
